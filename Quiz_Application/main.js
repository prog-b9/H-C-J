...
document.querySelector(".options-color").onclick = ()=>{
  document.querySelector(".colors").classList.toggle("show-colors");
};
...
// Select Options Colors
let colors = document.querySelectorAll(".colors .colors-content li");

// Get Local Storage For Colors
let colorLocalStorage = localStorage.getItem("colorItem");

if (colorLocalStorage != null) {

  // Save this color inside --main-color
  document.documentElement.style.setProperty("--main-color", colorLocalStorage);

  // Save Remove all Active
  colors.forEach((ele) => {
    ele.classList.remove("active");

    //  Save On Active Class
...
colors.forEach((lis) => {
  ...
  lis.addEventListener("click", (li) => {
    ...
    localStorage.setItem("colorItem", li.target.dataset.color);

    // Add Class Active On Click
    li.target.classList.add("active");

    // On Click li.traget Allready Reload the page bt
...
function getQuestions() {
  ...
  myRequest.onreadystatechange = () => {
    if (myRequest.readyState === 4 && myRequest.status === 200) {
      ...
      countDown(15,questionsOfNumber);
...
function showResults(obj, count) {
  ...
  if (counter === count) {
    ...
    if (rightAnswerBadGoodPerfect === count) {
      ...
      rightAnswerBadGoodPerfect > count / 2 &&
      rightAnswerBadGoodPerfect < count
    ) {
